...
knitr::include_graphics("docs/_main_files/figure-html/goalie.jpeg")
```

and theatre sports.

```{r impro, fig.cap='Did I hear someone laughing?...',echo=FALSE, out.width='80%', fig.asp=.75, fig.align='center'}
knitr::include_graphics("docs/_main_files/figure-html/improtheater.jpeg")
```
...
# library(kofvja)
# if(!exists("pcon") || !dbIsValid(pcon)) {
#   pcon <- createConObj(dbname = "kofdb",
#                        dbhost = "archivedb.kof.ethz.ch",
#                        passwd = .rs.askForPassword("Enter DB Password"))
#   }
# aggregates <- get_aggregates()
# 
# countries <- unique(c(aggregates$oecd,aggregates$eu,"cn"))
# 
# loading_vgr <- function(sector,connection){
#     tslist <- db_ts_read(con = connection,ts_keys=paste0("ch.kof.vja.kmm.",sector), schema = "vja")
#     return(tslist[[1]])
# }
# data_y<- loading_vgr("extot1", pcon)
exports <- read.xlsx("data/qna_e_csa.xls",sheetName="real_y", colIndex=c(1,27), startRow = 12, header=FALSE)
